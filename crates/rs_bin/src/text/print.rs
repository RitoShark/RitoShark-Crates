...
pub fn value_to_text(value: &BinValue, mapper: Option<&HashMapper>) -> String {
    // MERGE NOTE: the printer now threads a `Ctx` (mapper + TextOptions) instead of a bare
    ...
    // parent context, and TextOptions::default() leaves that rendering off anyway.
    push_value(&mut out, value, 0, &ctx, false);
    out
}
